...
def main():
    ...
    with slim.arg_scope(inception.inception_v1_arg_scope()):
        logits, _ = inception.inception_v1(images, num_classes=num_classes, is_training=is_training, dropout_keep_prob=args.dropout_keep_prob)
    ...
    #get = tf.get_default_graph().get_tensor_by_name('InceptionV1/Logits/Predictions/Softmax:0')
    #printOperationsName()
    #print(get)
    #exit()
    ...
    variables_to_restore = tf.contrib.framework.get_variables_to_restore(exclude=['InceptionV1/Logits/Predictions', 'InceptionV1/Logits/Conv2d_0c_1x1/'])
    init_fn = tf.contrib.framework.assign_from_checkpoint_fn(model_path, variables_to_restore)

    # Initialization operation from scratch for the new "fc8" layers
    # `get_variables` will only return the variables whose name starts with the given pattern
    fcPred_variables = tf.contrib.framework.get_variables('InceptionV1/Logits/Predictions')
    fcPred_init = tf.variables_initializer(fcPred_variables)

    fcConv0_variables = tf.contrib.framework.get_variables('InceptionV1/Logits/Conv2d_0c_1x1/')
    fcConv0_init = tf.variables_initializer(fcConv0_variables)
    ...
    sess.run(fcPred_init)  # initialize the new fc8 layer
    sess.run(fcConv0_init)  # initialize the new fc8 layer
    ...
    for epoch in range(num_epochs):
        ...
        train_acc, train_matrix = check_accuracy(sess, conf_matrix, is_training, train_init_op)
        val_acc, val_matrix = check_accuracy(sess, conf_matrix, is_training, val_init_op)
...
def check_accuracy(sess, conf_matrix, is_training, dataset_init_op):
    """
    ...
    while True:
        try:
            matrix += sess.run(conf_matrix, {is_training: False})
